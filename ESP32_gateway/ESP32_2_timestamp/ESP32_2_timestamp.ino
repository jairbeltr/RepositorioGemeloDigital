...
void notifyCallback(
  ...
  if (startIdx != -1 && endIdx != -1 && endIdx > startIdx) {
    ...
    if (!error) {
      ...
      // Enviar por MQTT al topic de STA2
      String jsonOut;
      serializeJson(jsonDoc, jsonOut);
      if (client.publish("sensor/STA2", jsonOut.c_str())) {
...
// 🔍 Conectar a STA2 BLE
void connectToSTA2() {
  Serial.println("🔍 Escaneando STA2...");
  pBLEScan->start(5, false);
  BLEScanResults results = *pBLEScan->getResults();
  for (int i = 0; i < results.getCount(); i++) {
    BLEAdvertisedDevice dev = results.getDevice(i);
    if (dev.getName() == "STA2_BLE") {
      Serial.println("✅ STA2_BLE encontrado, conectando...");
      ...
      if (svc) {
        ...
        if (ch) {
          ...
          Serial.println("✅ Conectado y suscrito a STA2_BLE");
          connected = true;
          return;
        }
      }
  ...
  Serial.println("❌ STA2_BLE no encontrado, reintentando...");
}
