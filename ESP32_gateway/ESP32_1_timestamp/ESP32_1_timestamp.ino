...
void notifyCallback(
  ...
  if (startIdx != -1 && endIdx != -1 && endIdx > startIdx) {
    ...
    if (!error) {
      ...
      if (client.publish("sensor/STA1", jsonOut.c_str())) {
...
// 🔍 Conectar a STA1 BLE
void connectToSTA1() {
  Serial.println("🔍 Escaneando STA1...");
  pBLEScan->start(5, false);
  BLEScanResults results = *pBLEScan->getResults();
  for (int i = 0; i < results.getCount(); i++) {
    BLEAdvertisedDevice dev = results.getDevice(i);
    if (dev.getName() == "STA1_BLE") {
      Serial.println("✅ STA1_BLE encontrado, conectando...");
      ...
      if (svc) {
        ...
        if (ch) {
          ...
          Serial.println("✅ Conectado y suscrito a STA1_BLE");
          connected = true;
          return;
        }
      }
  ...
  Serial.println("❌ STA1_BLE no encontrado, reintentando...");
}
